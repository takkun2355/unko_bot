...
class Roast(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    @commands.command(name="才能の無駄遣い")
    async def sainounomudazukai_command(ctx, arg: str = None):
        if arg == "home":
            """ランダムな質問をして返答をやりとり"""
            roasts = [
                    {
                        "question": "[緑色ポテトあった！！ラッキーなのか、、、、、？](https://twitter.com/Chaba_A15/status/1947874047976738857)",
                        "yes": "なるほど、それをラッキーだと思うのはいいと思う。情報の質は悪いけど。",
                        "no": "怖かったので一様すぐ捨てました！なので大丈夫です👍"
                    },
                    {
                        "question": "[すみません😣訳あってポストを消去したので再投稿です。](https://x.com/Chaba_A15/status/1951565925095973323)",
                        "yes": "https://x.com/Chaba_A15/status/1951565925095973323",
                        "no": "https://site.nanahoshi.akikukeo.f5.si/"
                    },
                    {
            ...
            await ctx.send("Welcom! smell the super-kusanggface-a=san 931!")
            ...
            roasts = [
                    {
                        ...
                        "next": "なるほど、吸収率の高さには納得がいく。情報の質は問わないと。"
                    },
                    {
                        "question": "君の脳、思考プロセッサというより、低品質な情報用のスポンジに見えるけど、自覚は？",
                        "next": "なるほど、吸収率の高さには納得がいく。情報の質は問わないと。"
            ...
            if response in ("enter", "go", "next"):
                await ctx.send(roast["next"])
            else:
                await ctx.send("Can't even answer a simple yes/no question? That's... not surprising.")
        elif arg == "c":
            """ランダムな質問をして返答をやりとり"""
            ...
            await ctx.send("❌ 無効な引数です。a, b, c のいずれかを指定してください。")
