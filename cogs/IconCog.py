...
class IconCog(commands.Cog):
    ...
    # =============================
    @commands.command(
        name="icon", help="指定したユーザーのアイコンを表示します（未指定で実行者）"
    )
    async def icon(
        ...
        embed.set_footer(
            text="サイズ=1024 を指定しています。アニメアイコンはGIFで返ります。"
        )

        await ctx.send(embed=embed)

    @commands.command(
        name="servericon", help="サーバーのアイコンを表示します（URLまたはID対応）"
    )
    async def servericon(self, ctx: commands.Context, target: Optional[str] = None):
        if not target:
            ...
            embed = discord.Embed(title=f"{guild.name} のサーバーアイコン").set_image(
                url=icon_url_hd
            )
            await ctx.send(embed=embed)
            return
        ...
        if target.isdigit():
            ...
            if guild and guild.icon:
                icon_url_hd = make_hd_url(guild.icon.url)
                embed = discord.Embed(
                    title=f"{guild.name} のサーバーアイコン"
                ).set_image(url=icon_url_hd)
                await ctx.send(embed=embed)
            else:
                await ctx.send(
        ...
        if "discord.gg" in target:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://discord.com/api/v10/invites/{invite_code}?with_counts=false"
                ) as resp:
                    if resp.status != 200:
                    ...
                    if guild_data and guild_data.get("icon"):
                        ...
                        embed = discord.Embed(
                            title=f"{guild_data['name']} のサーバーアイコン"
                        ).set_image(url=guild_icon)
                        await ctx.send(embed=embed)
                    else:
                        await ctx.send("このサーバーにはアイコンが設定されていません。")
            return
    ...
    @app_commands.describe(
        target="省略で現在のサーバー、または discord.gg/リンク またはサーバーID"
    )
    async def slash_servericon(
        ...
        await interaction.response.send_message(
            "サーバーを特定できませんでした。", ephemeral=True
        )
